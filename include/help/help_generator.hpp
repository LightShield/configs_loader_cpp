...
public:
    HelpGenerator(const ConfigsType& configs, const HelpFormat& format)
        : m_configs(configs), m_program_name(format.program_name), 
          m_max_width(format.max_width), m_use_colors(format.use_colors), 
          m_enable_interactive(format.enable_interactive), 
